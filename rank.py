...
from commonsense_sentence_generator import genCommonsenseSentence
...
def rankContext(roberta,rov,utterance,processed_commonsense,commonsense,extra=''):
	sentences = getSentences(processed_commonsense,rov)
	if "NoContext" in sentences:
		return "NoContext"
	else:
		gen_sentences = genCommonsenseSentence(utterance, commonsense)
		sentences.append(gen_sentences)
		print("Generated ", gen_sentences)
		if extra!='':
			for i in range(len(sentences)):
				if processed_commonsense in correct_phrase:
					replacement = processed_commonsense+' '+extra
				else:
					replacement = extra+' '+processed_commonsense
				if replacement not in sentences[i]:
					sentences[i] = sentences[i].lower().replace(processed_commonsense,replacement).capitalize()
		mostcontradictory = rank_sentences_based_on_contradiction(roberta,sentences,rov)
		x = mostcontradictory[1].capitalize()
		x = x.replace(' i ',' I ')
		print("Highest ranked ", x)
		return x
